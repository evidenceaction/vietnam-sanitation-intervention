...
calc.cost <- function(num.clust, clust.size, clust.cost=100, child.cost=5) {
  num.clust * clust.cost + num.clust * clust.size * child.cost 
}

calc.time.cost <- function(num.clust, clust.size, clust.cost=4, child.cost=0.5, day.length=10, num.teams=5*4) {
  num.clust * ceiling((clust.cost + clust.size * child.cost) / day.length) / num.teams
}
@


<<power-optimization-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, fig.cap="Visual Example of the Budget/Power Optimization Problem">>=
ggplot(power.grid.data) + 
  ...
  stat_function(aes(linetype="Financial"), fun=function(x) (250000 - x*100)/(x*5)) +
  stat_function(aes(linetype="Time"), fun=function(x) ((30*5*4*10/x) - 0.2)/(0.2)) + # linetype="dashed") +
#   geom_line(mapping=aes(x=x, y=y), data=data.frame(y=1:100) %>% mutate(x=(function(y, max.days=30, clust.cost=2, child.cost=0.2, day.length=12, num.teams=5*4) max.days * num.teams / ceiling((clust.cost + y * child.cost)/day.length))(y))) +
  ...
  scale_color_discrete(expression(MDE <= 0.05)) #"Enough Power (MDE < 0.05)")
@
