...
Assumptions
\begin{itemize}
  \item $ICC = 1/14$, from parasitology surveys in India
  \item Between cluster standard error = 0.3 (I'm just using a conservative value till I get a better estimate from India)
  \item Number of clusters = 1600
\end{itemize}

Power analysis is done for a significance level of 5\% and power of 80\%.

<<power-formula, echo=TRUE, tidy=TRUE>>=
...
<<infection-power-plot, echo=FALSE, fig.align='center', fig.height=3, fig.width=3>>=

qplot(1:50, 
      stat="function", 
      fun=function(cs) calc.mde(icc=0.07, num.clust=1600, clust.size=cs, residual=0.3), 
      geom="line",
      ylim=c(0.03,0.1),
      xlab="Cluster Size", ylab="Minimum Detectable Effect") #+ 
#   stat_function(fun=function(cs) calc.mde(icc=0.07, num.clust=1600, clust.size=cs, residual=0.2), linetype="dotted")
@

Therefore, if we survey 10 children from each cluster, we would have an MDE of about 4.5\% in prevalence.
