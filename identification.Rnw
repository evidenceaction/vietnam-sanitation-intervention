...
financial.clust.cost <- 80         # Team of 3
financial.child.cost <- 2.5 + 10   # $2.5/child (STH) + $10/child (HH survey)
time.clust.cost <- 4/24 + 3/24     # Travelling to communes takes 4 hours + 3 hr registration and selection (assumed fixed)  
time.child.cost <- 0.003           # 0.08 hr STH sampling/child (same day; lose half the cluster)

power.grid.data <- expand.grid(num.clust=seq(200, 1600), clust.size=2:200) %>%
  mutate(mde=vn.calc.mde(clust.size=round(clust.size/2), # Assuming 50% attrition of surveyed children
                         num.clust=num.clust),
...
calc.time.cost <- function(num.clust, clust.size, 
                           clust.cost=time.clust.cost,
                           child.cost=time.child.cost,
