...
power.grid.data <- expand.grid(num.clust=seq(200, 1600), clust.size=2:200) %>%
  mutate(mde=vn.calc.mde(clust.size=round(clust.size/2), num.clust=num.clust),
         mde.approx=round(mde, digits=3),
         low.power=mde > 0.05)

calc.cost <- function(num.clust, clust.size, 
                      clust.cost=80,         # Team of 3
                      child.cost=2.5 + 10) { # $2.5/child (STH) + $10/child (HH survey)
...
calc.time.cost <- function(num.clust, clust.size, 
                           clust.cost=4/24 + 3/24, # Travelling to communes takes 4 hours + 3 hr registration and selection (assumed fixed)  
                           child.cost=0.003, # 0.08 hr STH sampling/child (same day; lose half the cluster)
                           num.teams=2*4) { # Assuming two teams (of 3 members) per province
  num.clust * (clust.cost + clust.size * child.cost) / num.teams
}
...
time.budget.curve <- function(num.clust, budget,
  ...
  ((budget * num.teams / num.clust) - clust.cost)/child.cost
}

optim.num.clust <- uniroot(function(x) cost.budget.curve(x, financial.budget) - time.budget.curve(x, time.budget), c(200, 1600)) %>% use_series(root) %>% floor
optim.clust.size <- cost.budget.curve(optim.num.clust, financial.budget) %>% floor 
@


Figure \ref{fig:power-optimization-plot} shows the optimal number of clusters (= \Sexpr{optim.num.clust}) and cluster size (= \Sexpr{optim.clust.size}), giving an MDE of \Sexpr{sprintf("%.3f", vn.calc.mde(optim.clust.size, optim.num.clust))}.  This would cost ~\$\Sexpr{calc.cost(optim.num.clust, optim.clust.size) %>% format(big.mark=",")} and would take \Sexpr{calc.time.cost(optim.num.clust, optim.clust.size) %>% round} days.
