...
<<infection-power-plot, echo=FALSE, fig.align='center', warning=FALSE, fig.height=4, fig.width=6, eval=FALSE>>=
...
power.grid.data <- expand.grid(num.clust=seq(200, 800), clust.size=1:100) %>%
  group_by(clust.size) %>%
  ...
  mutate(mde=vn.calc.mde(clust.size=round(clust.size * child.compliance),  num.clust=num.clust),
         ...
         time.cost=calc.time.cost(num.clust, clust.size, num.clust.per.day.1, num.clust.per.day.2))
...
#   stat_function(aes(linetype="Time"), fun=time.budget.curve, args=list(budget=time.budget)) +
  geom_line(aes(x=num.clust, y=clust.size, linetype="Time"), data=power.grid.data %>% filter(time.cost == time.budget)) +
