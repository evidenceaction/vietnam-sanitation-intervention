...
<<power-formula, echo=FALSE, tidy=TRUE>>=
...
travel.between.time.hrs <- 0.5 
travel.to.time.hrs <- 2
day.length <- 14 - (travel.to.time.hrs * 2) + travel.between.time.hrs # length of work day - time to and from communes + double counted travel between time (below) 

financial.clust.cost <- 80            
financial.child.cost <- 2.5 + 10      # $2.5/child (STH) + $10/child (HH survey)

time.clust.cost.hrs <- ((travel.between.time.hrs * num.commune.visits) + 3) # Travelling one way to communes + 3 hr registration and selection (assumed fixed)  
time.child.cost.hrs <- 0.08 * num.commune.visits                     # 0.08 hr STH sampling/child 
...
calc.time.cost <- function(num.clust, clust.size, 
                           clust.cost=time.clust.cost.hrs,
                           child.cost=time.child.cost.hrs,
                           num.teams=num.sth.teams.per.province * num.target.provinces) { # Assuming two teams (of 3 members) per province
  total.cost.hrs <- (num.clust * (clust.cost + clust.size * child.cost)) 

  return(total.cost.hrs /(num.teams * day.length))
}
...
time.budget.curve <- function(num.clust, budget,
                           ...
                           child.cost=time.child.cost.hrs,
                           num.teams=num.sth.teams.per.province * num.target.provinces) { # Assuming two teams (of 3 members) per province
  ((budget * num.teams * day.length / num.clust) - clust.cost)/child.cost
}

power.grid.data <- expand.grid(num.clust=seq(200, 1600), clust.size=2:200) %>%
  mutate(mde=vn.calc.mde(clust.size=round(clust.size * child.compliance),  num.clust=num.clust),
         mde.approx=round(mde, digits=3),
         low.power=mde > 0.05,
         financial.cost=calc.cost(num.clust, clust.size),
         time.cost=calc.time.cost(num.clust, clust.size))

optim.rows <- power.grid.data %>% filter(time.cost <= time.budget, financial.cost <= financial.budget) %>% filter(mde == min(mde))
optim.num.clust <- optim.rows$num.clust[1]
optim.clust.size <- optim.rows$clust.size[1]
optim.mde <- optim.rows$mde[1]
@

Figure \ref{fig:power-optimization-plot} shows the optimal number of clusters (= \Sexpr{optim.num.clust}) and cluster size (= \Sexpr{optim.clust.size}), giving an MDE of \Sexpr{sprintf("%.3f", optim.mde)}.  This would cost \$\Sexpr{calc.cost(optim.num.clust, optim.clust.size) %>% format(big.mark=",")} and would take \Sexpr{calc.time.cost(optim.num.clust, optim.clust.size) %>% round} days.

<<power-optimization-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, fig.cap="Budget/Power Optimization Plot", cache=FALSE>>=
ggplot(power.grid.data) + 
  stat_contour(aes(x=num.clust, y=clust.size, z=mde, color=!low.power), breaks=unique(power.grid.data$mde.approx)) +
  stat_function(aes(linetype="Financial"), fun=cost.budget.curve, args=list(budget=financial.budget)) + 
  stat_function(aes(linetype="Time"), fun=time.budget.curve, args=list(budget=time.budget)) +
#   geom_line(mapping=aes(x=x, y=y), data=data.frame(y=1:100) %>% mutate(x=(function(y, max.days=30, clust.cost=2, child.cost=0.2, day.length=12, num.teams=5*4) max.days * num.teams / ceiling((clust.cost + y * child.cost)/day.length))(y))) +
#   geom_vline(aes(linetype="Time"), x=240) +
  geom_point(x=optim.num.clust, y=optim.clust.size) +
#   geom_text(label=sprintf("%d, %d)", optim.num.clust, optim.clust.size), x=optim.num.clust, y=optim.clust.size) +
  labs(x="Number of Clusters", y="Cluster Size") +
  coord_cartesian(ylim=c(0, 100)) +
  scale_linetype_discrete("Budget Curves") +
  scale_color_discrete(expression(MDE <= 0.05)) 
@
