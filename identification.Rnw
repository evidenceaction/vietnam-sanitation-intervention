...
ggplot(power.grid.data) + 
  stat_contour(aes(x=num.clust, y=clust.size, z=mde, color="mde"), binwidth=0.005) + #, breaks=unique(power.grid.data$mde.approx)) +
  stat_function(aes(color="financial"), fun=cost.budget.curve, args=list(budget=financial.budget)) + 
#   geom_ribbon(aes(x=num.clust, ymin=0, ymax=cost.budget.curve(num.clust, financial.budget), color="Financial"), alpha=0.25) + #, args=list(budget=financial.budget)) + 
#   geom_line(aes(x=num.clust, y=clust.size, linetype="Time"), data=time.budget.curve) +
#   geom_ribbon(aes(x=num.clust, ymin=clust.size.min, ymax=clust.size.max, color="time"), alpha=0.25, data=time.budget.curve) +
