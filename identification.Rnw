...
\usepackage{graphviz}
\usepackage[citestyle=authoryear, url=false, citetracker=true, maxcitenames=1, autopunct=true, backend=biber]{biblatex}
\usepackage{graphicx}
...
Assuming an intra-cluster correlation coefficient of 0.04, as found by \textcite{Freeman2013} for \emph{A. lumbricoides}, and conducting power analysis for a significance level of 5\% and power of 80\% we have the following plot of Minimum Detectable Effect (MDE)
...
vn.calc.mde <- function(clust.size, num.clust=1600) calc.mde(icc=0.04, num.clust=num.clust, clust.size=clust.size, residual=0.16)
...
Therefore, if we have 1600 clusters and survey 10 children per cluster, we would have an MDE of \Sexpr{vn.calc.mde(10)}\% in prevalence.
